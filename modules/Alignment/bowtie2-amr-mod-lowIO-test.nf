...
process bowtie2_align {
	...
	${SAMTOOLS} view -bS ${pair_id}_alignment.sam | ${SAMTOOLS} sort -@ ${threads} -o ${pair_id}_alignment.bam
...
process bowtie2_rm_contaminant_fq {
    ...
    ${SAMTOOLS} view -h -f 12 -b ${pair_id}.host.sorted.bam | ${SAMTOOLS} sort -n -@ ${threads} -o - | \
	    ${SAMTOOLS} fastq -@ ${threads} -c 6 -1 ${pair_id}.non.host.R1.fastq.gz -2 ${pair_id}.non.host.R2.fastq.gz -0 /dev/null -s /dev/null -n -
